...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.d("TAG", "error 1") // tambahkan perintah ini
        searchTanaman.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
            override fun onQueryTextSubmit(query: String): Boolean {
                Log.d("TAG", "error 2") // tambahkan perintah ini
                return false
                Log.d("TAG", "error 3") // tambahkan perintah ini
            }

            override fun onQueryTextChange(newText: String): Boolean {
                mainAdapter.filter.filter(newText)
                Log.d("TAG", "error 4") // tambahkan perintah ini
                return true
                Log.d("TAG", "error 5") // tambahkan perintah ini
            }
        })
...
//        mainAdapter = MainAdapter(this, modelMain)
        ...
        Log.d("TAG", "error 6") // tambahkan perintah ini
//        rvListTanaman.adapter = mainAdapter

        // get data json
        getNamaTanaman()
        Log.d("TAG", "error 7") // tambahkan perintah ini
    }
...
//                mainAdapter.notifyDataSetChanged()
                mainAdapter = MainAdapter(this, modelMain)
                Log.d("TAG", "error 8") // tambahkan perintah ini
                rvListTanaman.adapter = mainAdapter
                Log.d("TAG", "error 9") // tambahkan perintah ini
